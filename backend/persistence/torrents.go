...
INSERT INTO torrents (infohash, name, magnet, save_path, category_id, added_at, completed_at, paused, queue_position, force_start, metainfo, sequential)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
`, r.InfoHash, r.Name, r.Magnet, r.SavePath, catID, r.AddedAt.Unix(), completed, paused, r.QueuePosition, forceStart, nullableBytes(r.Metainfo), sequential)
	return err
}
...
SELECT infohash, name, COALESCE(magnet, ''), save_path, category_id, added_at, completed_at, paused, queue_position, force_start, metainfo, sequential
...
func scanTorrent(s scanner) (TorrentRecord, error) {
	...
	var sequential int
	var metainfo []byte
	if err := s.Scan(&r.InfoHash, &r.Name, &r.Magnet, &r.SavePath, &categoryID, &addedAt, &completedAt, &paused, &r.QueuePosition, &forceStart, &metainfo, &sequential); err != nil {
