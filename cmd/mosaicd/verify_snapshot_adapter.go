...
// verifySnapshotAdapter mirrors the GUI's adapter (see ../../verify_snapshot_adapter.go).
// It bridges the persistence DAO and the engine's SnapshotStore interface.
// Errors are logged and swallowed: a missing or unreadable snapshot just means
// we fall back to the full piece-by-piece verify, and a failed Save just means
// the next startup has to re-verify once.
